...
public class ParcelleService implements IParcelleService {
	...
	public static final Logger LOGGER = Logger.getLogger(String.valueOf(ParcelleService.class));
	...
	public boolean suppressionParcelles(List<String> ids) {
		LOGGER.info("Nombre de parcelles à supprimer : " + ids.size());
		int nbParcellesSupprimees = parcelleRepository.updateSupprimeByIdIn(ids);
		LOGGER.info("Nombre de parcelles supprimées : " + nbParcellesSupprimees);
	...
	public void enregistrementParcelle(Parcelle parcelle) {
		LOGGER.info("Enregistrement de la parcelle " + parcelle.getId());
		parcelleRepository.save(parcelle);
		LOGGER.info("Fin enregistrement de la parcelle " + parcelle.getId());
	}
